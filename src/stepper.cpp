...
double acc_frenquence[501];
int time_delay[501];
int DriveTimeDelay[501];
int TwistTimeDelay[501];
int RaceTimeDelay[501];
...
// 以下代码用于生成加速阶段的脉冲触发时刻
// 运动参数（可根据实际情况调整）：
const float T_accel = 0.5;       // 加速阶段总时长（单位：秒）
const float v_max = 1000.0;      // 理论最大速度（步/秒）
...
const unsigned int MAX_STEPS = 300;  // 预估加速阶段内的最大步数
unsigned long stepTimes[MAX_STEPS];
...
void generateSCurveStepTimes() {
    ...
    while(t <= T_accel && numSteps < MAX_STEPS) {
        ...
        if (floor(pos) > lastStepPos) {
            stepTimes[numSteps++] = (unsigned long)(t * 1000000.0); // 转换为微秒
...
void Pulse_Sender(int pin, int num) {
    if (num > 2 * Pulse_Acc)
    {
        ...
        {
            if (robot.l.isTight == 0)
                ChosenMode = RaceTimeDelay;
            else if (robot.r.isTight == 1)
                ChosenMode = TwistTimeDelay;
            else if (robot.r.isTight == 0)
        ...
        for (int i = 1; i <= Pulse_Acc; i++)
            PULSE_GENERATOR(pin, ChosenMode[i]);

        for (int i = 1; i <= num - 2 * Pulse_Acc; i++)
            PULSE_GENERATOR(pin, ChosenMode[Pulse_Acc]);
